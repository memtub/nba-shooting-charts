const EASTERN_CONFERENCE_TEAMS = [
    "Boston Celtics", "Brooklyn Nets", "New York Knicks", 
    "Philadelphia 76ers", "Toronto Raptors", "Atlanta Hawks", 
    "Charlotte Hornets", "Miami Heat", "Orlando Magic", 
    "Washington Wizards", "Cleveland Cavaliers", "Indiana Pacers", 
    "Detroit Pistons", "Milwaukee Bucks", "Chicago Bulls"
];

const WESTERN_CONFERENCE_TEAMS = [
    "Los Angeles Lakers", "Los Angeles Clippers", "Phoenix Suns", 
    "Sacramento Kings", "Golden State Warriors", "Denver Nuggets", 
    "Minnesota Timberwolves", "Oklahoma City Thunder", 
    "Portland Trail Blazers", "Utah Jazz", "Dallas Mavericks", 
    "Houston Rockets", "Memphis Grizzlies", "New Orleans Pelicans", 
    "San Antonio Spurs"
];